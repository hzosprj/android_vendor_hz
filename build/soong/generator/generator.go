...
func init() {
	android.RegisterModuleType("hz_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hzExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hzExpandVariables(ctx, dep_file)
	...
	cmd := hzExpandVariables(ctx, String(g.properties.Cmd))
