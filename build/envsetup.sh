function __print_hz_functions_help() {
cat <<EOF
Additional HZOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- hzgerrit:   A Git wrapper that fetches/pushes patch from/to HZOS Gerrit Review.
- hzrebase:   Rebase a Gerrit change and push it again.
- hzremote:   Add git remote for HZOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for HZOS Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/hz/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the hz model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch hz_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/hz-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.hz.device | grep -q "$HZ_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $HZ_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function hzremote()
{
    ...
    local HZ_USER=$(git config --get review.review.hzos.org.username)
    if [ -z "$HZ_USER" ]
    then
        git remote add hz ssh://review.hzos.org:29418/$PFX$PROJECT
    else
        git remote add hz ssh://$HZ_USER@review.hzos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'hz' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/HZOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.hz.device | grep -q "$HZ_BUILD");
    then
...
function hzgerrit() {
    ...
    local user=`git config --get review.review.hzos.org.username`
...
function hzrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "HZOS Gerrit Rebase Usage: "
        echo "      hzrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.hzos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.hz.device | grep -q "$HZ_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/hz/build/tools/repopick.py $@
}
