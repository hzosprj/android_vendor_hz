...
TOOLS_PATH_OVERRIDE := \
    PERL5LIB=$(BUILD_TOP)/prebuilts/tools-hz/common/perl-base
...
ifneq ($(KERNEL_NO_GCC), true)
    ...
    ifeq ($(KERNEL_ARCH),arm64)
        ...
        TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-hz/$(HOST_PREBUILT_TAG)/bin:$(KERNEL_TOOLCHAIN_arm):$$PATH
    else
        TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-hz/$(HOST_PREBUILT_TAG)/bin:$$PATH
    endif
    ...
    TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-hz/$(HOST_PREBUILT_TAG)/bin:$(TARGET_KERNEL_CLANG_PATH)/bin:$$PATH
endif
